...
--
-- Achado ao aplicar em produção: a versão original usava uma temp table
-- `on commit drop` + UPDATE + INSERT em 3 statements separados. Uma temp
-- table "on commit drop" só sobrevive até o fim da transação ATUAL — e a
-- ferramenta usada para rodar a migration executava cada statement em
-- autocommit (uma transação por comando), então a tabela já tinha sumido
-- antes do UPDATE seguinte rodar ("relation _unlinked does not exist").
-- Corrigido para uma única instrução atômica: UPDATE...RETURNING dentro de
-- um CTE alimentando o INSERT — não depende de como a ferramenta faz o
-- batching entre statements.
--
